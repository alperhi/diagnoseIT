...
/**
 * A <code>Tag</code> represents the actual result of a rule execution.
 * Everything what is created by a {@link ActionMethod} is wrapped in a
 * <code>Tag</code>. A <code>Tag</code> can thus be considered as a qualified
 * attachment to the currently analyzed object. Depending on the
 * {@link ActionMethod} configuration/implementation, one rule can produce
 * multiple <code>Tag</code> s. <b>Be aware that all produced Tags are of the
 * same type. It is invalid/impossible to create different Tag types</b>. All
 * <code>Tag</code>s are gathered to one {@link RuleOutput} and stored in the
 * {@link IRuleOutputStorage}.
 *
...
public class Tag {
	 ...
	 *
	 * @param type
	 *            The type of this <code>Tag</code>. The is an equivalent to a
	 *            Tag's name.
	 */
	 ...
	 *            The value of this <code>Tag</code>
	 * @param parent
	 *            The <code>Tag</code> which is considered as the predecessor of
	 *            this <code>Tag</code>.
	 */
	...
	/**
	 * Utility method to mark a <code>Tag</code> as parent by setting the state
	 * to {@link TagState#PARENT}.
	 */
	...
	public String toString() {
		return new ToStringBuilder(this).append("type", type)
				.append("value", value).append("parent", parent)
	...
	public boolean equals(Object o) {
		...
		return new EqualsBuilder().append(getType(), tag.getType())
				.append(getValue(), tag.getValue())
				.append(getParent(), tag.getParent())
				.append(getState(), tag.getState()).isEquals();
	}

	@Override
	public int hashCode() {
		return new HashCodeBuilder(17, 37).append(getType()).append(getValue())
				.append(getParent()).append(getState()).toHashCode();
	}
}
