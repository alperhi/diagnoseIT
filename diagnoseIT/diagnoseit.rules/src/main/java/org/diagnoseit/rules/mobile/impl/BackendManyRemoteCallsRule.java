...
public class BackendManyRemoteCallsRule {
	...
	// LoggerInitializer.getLogger(BackendManyRemoteCallsRule.class.getName());
	//
	// private static final double NUMBER_OF_REMOTE_CALLS_IN_A_SECOND =
	// AntiPatternConfig.getInstance().getPropertyDouble("BACKEND_MANY_REMOTE_CALLS_RULE_NUMBER_OF_REMOTE_CALLS_IN_A_SECOND");
	//
	// private static final int MIN_AMOUNT_OF_CALLS =
	// AntiPatternConfig.getInstance().getPropertyInt("BACKEND_MANY_REMOTE_CALLS_RULE_MIN_AMOUNT_OF_CALLS");
	//
	...
	//
	// if ((remoteInvocations.size() >= MIN_AMOUNT_OF_CALLS) && (remoteInvocations.size() >=
	// (completeDurationOfSubtraces * NUMBER_OF_REMOTE_CALLS_IN_A_SECOND))) {
	// log.info("Java application executed too many remote calls. The amount of the calls is " +
	// remoteInvocations.size() + ". Total time of the Java agent traces is: "
